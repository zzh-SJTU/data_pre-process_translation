...
WMT14_test=Custom_dataset(source_test_dir,target_test_dir)
...
def evaluate(model):
    ...
    WMT14_test=Custom_dataset(source_test_dir,target_test_dir)
    #train, valid, test = torchtext.datasets.IWSLT2016(language_pair=(SRC_LANGUAGE, TGT_LANGUAGE))
    val_dataloader = DataLoader(WMT14_test, batch_size=BATCH_SIZE, collate_fn=collate_fn)
...
refe=[]
bleu = BLEU()
for sour,targ in WMT14_test :
    tar_list=[targ]
    hypo.append(translate(transformer, sour))
    refe.append(tar_list)
bleu_score=bleu.corpus_score(hypo, refe)
